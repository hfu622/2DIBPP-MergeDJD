    ...
    public NestPath canCombine(NestPath nonConvex, NestPath convex) {
        ...
        for(int i = 0; i < nonConvex.size(); i++){
            ...
            for(int j = 0; j < convex.size(); j++){
                ...
                // 旋转移动后的多边形，旋转四次即可
//                rotateShifPath(shifPath, j, nonConvex, i);
...
//                    }
    ...
    /**
     * shifpath是移动后的凸多边形，其第j个顶点和nonconvex的第i个顶点对齐
     * 通过旋转shifpath来使得两个多边形不重叠
     */
    private NestPath rotateShifPath(NestPath shifPath, int j, NestPath nonConvex, int i) {
        int n = shifPath.size(); //n个顶点
        //找到前一个顶点和后一个顶点
        Segment shifPoint = shifPath.get(j); //当前点a
        Segment prev1 = shifPath.get((j - 1) % n); // b
        Segment succ1 = shifPath.get((j + 1) % n); // b

        //找到前一个顶点和后一个顶点
        Segment nonPoint = nonConvex.get(i); //当前点c
        Segment prev2 = nonConvex.get((i - 1) % n); // d
        Segment succ2 = nonConvex.get((i + 1) % n); // d
    ...
    private double calculateFitness(NestPath newConvex, NestPath nonConvex, NestPath convex, NestPath shifPath) {
        // shifPath是由convex移动来的。函数的每一个参数都是一个多边形
        // todo: 我感觉不用加权重，因为你也不知道哪个重要，只设计一个参数，就是fitness的接受阈值，会慢慢降低
        double fitness = 0;
        int numOfNewConvex = newConvex.size(); // 多边形点数
